type 'a cell = {
  mutable prev : 'a cell ref option;
  mutable next : 'a cell ref option;
  value : 'a;
}

type 'a t = {
  mutable first : 'a cell ref option;
  mutable cnt : 'a cell option;
  mutable last : 'a cell ref option;
}
...
let add is_first e pl =
  let dp = ref { prev = None; value = e; next = None } in
